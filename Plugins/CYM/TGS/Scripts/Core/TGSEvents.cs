...
namespace CYM.TGS
{
    ...
    public delegate void CellEvent(TGS sender, int cellIndex);
    public delegate void CellHighlightEvent(TGS sender, int cellIndex, ref bool cancelHighlight);
    public delegate void CellClickEvent(TGS sender, int cellIndex, int buttonIndex);
    public delegate void CellDragEvent(TGS sender, int cellOriginIndex, int cellTargetIndex);

    public delegate void TerritoryEvent(TGS sender, int territoryIndex);
    public delegate void TerritoryRegionEvent(TGS sender, int territoryIndex, int regionIndex);
    public delegate void TerritoryHighlightEvent(TGS sender, int territoryIndex, ref bool cancelHighlight);
    public delegate void TerritoryClickEvent(TGS sender, int territoryIndex, int buttonIndex);
    public delegate void TerritoryRegionClickEvent(TGS sender, int territoryIndex, int regionIndex, int buttonIndex);

    public delegate int PathFindingEvent(TGS sender, int cellIndex);

    public delegate void OnGridSettingsChangedEvent(TGS tgs);

    public delegate void OnRectangleSelectionEvent(TGS sender, Vector2 localStartPos, Vector2 localEndPos);
