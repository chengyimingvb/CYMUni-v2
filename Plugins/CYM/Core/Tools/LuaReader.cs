...
namespace CYM
{
    ...
    public static class LuaReader
    {
        ...
        public static string FinalNamespace = "NationWar";
        ...
        private static DiffData ReadDiffData(Table luaTable)
        {
            ...
            {
                dictionary[Enum<GameDiffType>.Parse(pair.Key.String)] = (float)pair.Value.Number;
            }
            return dictionary;
        }
        ...
        private static object CreateObj(Table luaTable, Type type, out Type newType)
        {
            ...
            try
            {
                ...
                if (temp != null)
                {
                    newType = AssemblyCSharp.GetType(nameSpace + "." + temp.String, false, false);
                    if (newType == null)
                    {
                        newType = AssemblyCSharp.GetType(FinalNamespace + "." + temp.String, true, false);
                        if (newType == null)
                            Debug.LogError("无法找到此类型:" + temp);
                    }
