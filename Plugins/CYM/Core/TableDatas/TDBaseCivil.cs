...
namespace CYM
{
    ...
    public class GenderInfo
    {
        ...
        public void Parse()
        {
            ...
            void ParseInternal(PartConfig iConfig, List<PartInfo> pConfig, Dictionary<string, HashList<PartInfo>> sConfig)
            {
                ...
                {
                    var data = BaseGlobal.GRMgr.Head.GetStrsByCategory(iConfig.Director);
                ...
                foreach (var pConfigItem in pConfig)
                {
                    ...
                    Enum<AgeRange>.For(x =>
                    {
                        ...
                        if (BaseGlobal.GRMgr.Head.IsHave(addtionName))
                        {
    ...
    {
        public string Director { get; set; } = Const.STR_Inv;
        public string PrefixName { get; set; } = Const.STR_Inv;
        public int PrefixCount { get; set; } = Const.INT_Inv;
