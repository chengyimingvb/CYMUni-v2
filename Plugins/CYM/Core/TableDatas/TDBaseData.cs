...
namespace CYM
{
    ...
    public class TDBaseData : IBase, ICloneable, ITDBaseData
    {
        ...
        //int id
        public int INID { get; set; } = Const.INT_Inv;
        //剧本ID
        public string Drama { get; set; } = Const.STR_Inv;
        //分组ID
        public string Group { get; set; } = Const.STR_Inv;
        public int Repeat { get; set; } = Const.INT_Inv;
        //和所有人都是敌人的单位(盗贼,野怪等)
        ...
        public virtual string GetDesc(params object[] ps)
        {
            ...
            return BaseLangMgr.Get(Const.STR_Desc_NoDesc);
        }
        public virtual string GetCont()
        {
            if (!string.IsNullOrEmpty(Cont))
                return BaseLangMgr.Get(Cont);
            string descTDID = Const.Prefix_Cont + TDID;
            if (BaseLangMgr.IsContain(descTDID))
        ...
        public virtual Sprite GetIllstration()
        {
            if (Icon == CustomInvalidID)
                return null;
            if (!Icon.IsInv())
                return GRMgr.Illustration.Get(Icon, false);
            else if (!TDID.IsInv() && GRMgr.Illustration.IsHave(TDID))
                return GRMgr.Illustration.Get(TDID, false);
            return null;
        }
        // 获得禁用的图标,有可能没有
        ...
        where TConfig : TDBaseConfig<TConfigData>,new()
    {
        ...
        //public TDBaseGlobalConfig(string keyName):base(keyName)
        //{
        //    Ins = this as TConfig;
        //}
        #endregion
