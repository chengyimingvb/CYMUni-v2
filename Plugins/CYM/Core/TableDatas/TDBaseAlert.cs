...
namespace CYM
{
    ...
    public class TDBaseAlertData : TDBaseData
    {
        ...
        #region prop
        //Self条款
        public List<TDBaseArticleData> SelfArticle { get; private set; } = new List<TDBaseArticleData>();
        //Target条款
        public List<TDBaseArticleData> TargetArticle { get; private set; } = new List<TDBaseArticleData>();
        //战争
        public IWarfareData WarfareData { get; set; } = null;
        //额外参数
        ...
        #region get
        public Sprite GetBg() => GRMgr.Icon.Get(Bg);
        public Sprite GetIllustration() => GRMgr.Illustration.Get(Illustration);
        ...
        {
            string tipKey = Const.Prefix_Tip + baseKey;
            string detailKey = Const.Prefix_Desc + baseKey;
        ...
        public virtual void ExecuteArticle()
        {
            if (SelfArticle != null)
            {
                foreach (var item in SelfArticle)
                    item.OnExecute?.Invoke(item);
            }
            if (TargetArticle != null)
            {
                foreach (var item in TargetArticle)
                    item.OnExecute?.Invoke(item);
            }
        }
        #endregion
