...
namespace CYM
{
    ...
    public class BaseSceneRoot : BaseMono
    {
        ...
        void BakePathfindingMap()
        {
            ...
            texture.Apply();
            FileUtil.SaveTextureToPNG(texture, Path.Combine(Const.Path_ResourcesTemp, nameof(BakePathfindingMap)+".png"));
        }
        #endregion
            ...
            if (Application.isPlaying)
            {
                if (InstantiateOnPlay != null && BaseGlobal.BattleMgr != null)
                {
                    foreach (var item in InstantiateOnPlay)
                    {
                        ...
                        TGS.TGS tgs = tempGO.GetComponent<TGS.TGS>();
                        if (tgs != null)
                        {
                            TGS = tgs;
                        }
