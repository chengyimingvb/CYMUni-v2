...
namespace CYM
{
        ...
        where TOwner : BaseUnit
    {
        ...
        public override bool IsPlayer() => ScreenMgr.Player == Owner;
        ...
        public override bool IsFriend(BaseUnit other)
        {
            if (IsMainlyUnit)
            {
                if (BaseGlobal.RelationMgr == null)
                {
                    CLog.Error("错误!Mainly Unit 必须依赖 组件");
                    return false;
                }
                return BaseGlobal.RelationMgr.IsFriend(this, other.BaseOwner);
            }
            else
            {
        ...
        public override bool IsEnemy(BaseUnit other)
        {
            if (IsMainlyUnit)
            {
                ...
                return BaseGlobal.RelationMgr.IsInWarfare(this, other.BaseOwner);
            }
            else
            {
