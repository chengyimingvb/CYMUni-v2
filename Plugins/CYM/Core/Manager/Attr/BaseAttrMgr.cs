...
namespace CYM
{
        ...
        where T : Enum
    {
        ...
        public override void Refresh()
        {
            ...
            {
                if (_percentData[i].Valid) AddAttrVal(_percentData[i].Type, _percentData[i].RealVal * _getBaseAttrVal(Enum<T>.Int(_percentData[i].Type)));
            }
            //额外加成
                ...
                foreach (var item in _attrConvertData)
                {
                    if (item != null)
                    {
                        if (item.FactionType == AttrFactionType.DirectAdd)
                            ...
                            AddAttrVal(item.To, GetExtraAddtion(item) * _getBaseAttrVal(Enum<T>.Int(item.To)));
                    }
                }
            }
        ...
        public Sprite GetIcon(T type)
        {
            if (attrDataList == null)
                return null;
            return attrDataList[Enum<T>.Int(type)].GetIcon();
        }
        // 获得额外的加成
        ...
        public string GetConvertStr(T from, string Indent = "", bool isNewline = true, float? customVal = null)
        {
            ...
            if (_attrConvertData != null)
            {
                foreach (var item in _attrConvertData)
                {
                    if (item != null && Enum<T>.Int(item.From) == Enum<T>.Int(from))
                    {
        ...
        public static string GetAttrStr(T Type, float Val, bool isPercent = false, bool isIgnoreName = false, bool isColor = true)
        {
            ...
            //组合
            string str1 = name + Const.UCD_NoBreakingSpace + strVal;
            //属性颜色
        ...
        public static string GetAttrNumberStr(T Type, float Val)
        {
            ...
            if (tempAttrData != null)
            {
                var tempData = tempAttrData[Enum<T>.Int(Type)];
        ...
        {
            return GetAttrDataList()[Enum<T>.Int(type)].GetIcon();
        }
        /// <summary>
        /// 获得属性颜色
        /// </summary>
        ...
        public static string GetAttrColor(T Type, float Val, bool isReverse = false)
        {
            ...
            if (tempAttrData != null)
            {
                ...
                {
                    color = Const.CH_Yellow;
                }

                if (isReverse)
                {
                    if (color.Contains(Const.COL_Green))
                    {
                        color.Replace(Const.COL_Green, Const.COL_Red);
                    }
                    else if (color.Contains(Const.COL_Red))
                    {
                        color.Replace(Const.COL_Red, Const.COL_Green);
                    }
                }
            }
            return color;
