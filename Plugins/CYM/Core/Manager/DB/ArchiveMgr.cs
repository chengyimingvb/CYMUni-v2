...
namespace CYM
{
    public class ArchiveMgr<T> : IArchiveMgr where T : DBBaseGame, new()
    {
        ...
        // 得到存档路径
        string GetArchivePath(string name) => Path.Combine(_basePath, name + Const.Extention_Save);
        // 得到所有文件
        public string[] GetFiles() => FileUtil.GetFiles(_basePath, "*" + Const.Extention_Save, SearchOption.AllDirectories);
        #endregion
