...
namespace CYM
{
        ...
        where TGameData : DBBaseGame, new()
    {
        ...
        public string GetTempSavePath()=> Path.Combine(Const.Path_LocalDB, Const.STR_DBTempSaveName + Const.Extention_Save);
        #endregion
        ...
        public void AutoSave(bool useSnapshot = false, bool isForce = false)
        {
            ...
            SaveAs(Const.Prefix_AutoSave + Util.GetStr(BaseGlobal.Player.TDID), useSnapshot, true, false, false);
        }
        public void SaveTemp(bool useSnapshot = false, bool isAsyn=false)
        {
            SaveAs(Const.STR_DBTempSaveName, useSnapshot, isAsyn, false, true);
        }
        // 加载游戏
