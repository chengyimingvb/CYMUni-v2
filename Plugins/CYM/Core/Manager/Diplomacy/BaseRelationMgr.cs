...
namespace CYM
{
    ...
    public class BaseRelationMgr<TUnit, TWarData> : BaseGFlowMgr, IRelationMgr
        where TUnit : BaseUnit
        where TWarData : BaseWarfareData<TUnit>, IWarfareData, new()
    {
        const int MaxRelationShip = 100;
        const int MinRelationShip = -100;
        public RangeDic<RelationShipType> RelationShipTypeData = new RangeDic<RelationShipType>()
        {
            { RelationShipType.Close,       new Range(90,MaxRelationShip) },
            { RelationShipType.Friendly,    new Range(10,90) },
            { RelationShipType.Neutral,     new Range(-10,10) },
            { RelationShipType.Disgust,     new Range(-50,-10) },
            { RelationShipType.Feud,        new Range(MinRelationShip,-50) },
        };
        ...
        #region life
        //停战期限
        protected virtual int ArmisticeCD => 10;
        ...
        public RelationShipType GetRelationShipType(int val)
        {
            return RelationShipTypeData.Get(val);
        }
        public RelationShipType GetRelationShipType(TUnit first, TUnit second)
        {
            return GetRelationShipType(GetRelationShip(first, second));
        }
        #endregion
        ...
        #region db
        public List<DBBaseWar> GetWarDBData()
        {
            List<DBBaseWar> ret = new List<DBBaseWar>();
            foreach (var item in Warfare)
            {
                DBBaseWar data = new DBBaseWar();
                data.ID = item.ID;
                data.TDID = item.TDID;
                ...
                item.AllowLand.ForEach(x => data.AllowOccupy.Add(x.BaseConfig.TDID));
                ret.Add(data);
            }
            return ret;
        }
        public void LoadWarDBData(List<DBBaseWar> data)
        {
            if (data == null) return;
            foreach (var item in data)
            {
                ...
                item.AllowOccupy.ForEach(x => war.AllowLand.Add(GetEntity<TUnit>(x)));
            }
        }
        public MultiDic<string, int> GetRelationShipDBData()
        {
            MultiDic<string, int> ret = new MultiDic<string, int>();
        ...
        public MultiDic<string, bool> GetAllianceDBData()
        {
            MultiDic<string, bool> ret = new MultiDic<string, bool>();
        ...
        public MultiDic<string, bool> GetMarriageDBData()
        {
            MultiDic<string, bool> ret = new MultiDic<string, bool>();
        ...
        public MultiDic<string, int> GetArmisticeDBData()
        {
            MultiDic<string, int> ret = new MultiDic<string, int>();
        ...
        public Dictionary<string, HashList<string>> GetVassalDBData()
        {
            Dictionary<string, HashList<string>> ret = new Dictionary<string, HashList<string>>();
            Vassal.ForEach((x, y) =>
            {
