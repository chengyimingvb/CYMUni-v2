...
namespace CYM
{
    public class BaseLangMgr : BaseGFlowMgr, ILoader
    {
        ...
        public static List<string> LoadTips => GetCategoryKeyList(Const.Lang_Category_LoadTip);
        ...
        public static string Space
        {
            get
            {
                if (LanguageType == LanguageType.English ||
                    ...
                    return Const.UCD_NoBreakingSpace;
                return "";
            }
        }
        #endregion
        ...
        {
            if (key.IsInv()) return "";
            if (curDic == null) return Const.STR_Unkown;
        ...
        public virtual void Add(LanguageType? type, string key, string desc, string fileName = "", string category = "")
        {
            ...
            if (!data[lanIndex].ContainsKey(key))
            {
                ...
                if (key.StartsWith(Const.Prefix_Lang_Surname))
        ...
        public IEnumerator Load()
        {
            ...
            foreach (var dlc in DLCManager.LoadedDLCItems.Values)
            {
                ...
                {
                    var assetBundle = dlc.LoadRawBundle(Const.BN_Language);
                    if (assetBundle != null)
                    {
                    ...
                    else
                    {
                        ...
                        for (int colIndex = 0; colIndex < NumberOfCols; ++colIndex)
                        {
                            ...
                            if (colIndex == 0)
                            {
                                ...
                                //跳过分类
                                else if (key.StartsWith(Const.Prefix_Lang_Category))
                                {
                                    lastedCategory = key.Remove(0, 1);
                                    continue;
                                }
                                //自动ID
                                else if (key == Const.Prefix_Lang_AutoID)
                                {
