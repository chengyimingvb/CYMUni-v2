...
namespace CYM
{
        ...
        where TData : TDBaseAlertData, new()
    {
        ...
        #region prop
        IArticleMgr<TDBaseArticleData> ArticleMgr => BaseGlobal.ArticleMgr;
        ...
        public void Remove(TData alert)
        {
            ...
            foreach (var item in alert.SelfArticle)
                ArticleMgr.RemoveArticle(item.ID);
        ...
        private bool IsCanAddDisposableAlert()
        {
            ...
            if (Options.Ins.IsAllAlert) return true;
        ...
        public void LoadDBData(ref List<DBBaseAlert> data)
        {
            ...
            foreach (var item in data)
            {
                ...
                alert.WarfareData = RelationMgr.GetBaseWarfareData(item.War);
                item.SelfArticle.ForEach(x => alert.SelfArticle.Add(ArticleMgr.Get(x)));
                item.TargetArticle.ForEach(x => alert.TargetArticle.Add(ArticleMgr.Get(x)));
            }
        }
        #endregion
    }
