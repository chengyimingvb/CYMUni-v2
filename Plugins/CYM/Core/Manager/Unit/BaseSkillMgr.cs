...
namespace CYM
{
    public class BaseSkillMgr<TData> : BaseMgr, IOnAnimTrigger, IPrespawner where TData : TDBaseSkillData, new()
    {
        ...
        {
            if (!ITDConfig.Contains(skillName))
                return;
            TData tempSkill = ITDConfig.Get<TData>(skillName).Copy<TData>();
            if (tempSkill != null)
            {
