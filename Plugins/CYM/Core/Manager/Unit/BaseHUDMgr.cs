...
namespace CYM
{
        ...
        where T : UHUDBar
    {
        ...
        protected virtual string BarItemPrefab => Const.STR_Inv;
        ...
        void UpdateJumpFontList()
        {
            if (jumpFontTimer.Elapsed() > NextInterval)
            {
                ...
                {
                    JumpFont(jumpList[0]);
                    var go = BaseGlobal.GRMgr.UI.Get(jumpList[0].Prefab);
        ...
        {
            if (SelfBaseUnit == null) return null;
            if (SelfBaseUnit.NodeMgr == null) return SelfBaseUnit.Trans;
            return SelfBaseUnit.NodeMgr.GetBone(type);
        }
        #endregion
