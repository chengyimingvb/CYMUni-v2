...
namespace CYM.Pool
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        private void Despawn(Transform instance)
        {
            ...
            instance.position = Const.VEC_FarawayPos;
        ...
        public Transform SpawnTrans(GameObject go, Vector3? position = null, Quaternion? quaternion = null, Transform paraent = null)
        {
            ...
            Transform ret = null;
            if (paraent == null)
                ret = Spawn(go, position.HasValue ? position.Value : Const.VEC_FarawayPos, quaternion.HasValue ? quaternion.Value : Quaternion.identity);
            else
                ret = Spawn(go, position.HasValue ? position.Value : Const.VEC_FarawayPos, quaternion.HasValue ? quaternion.Value : Quaternion.identity, paraent);
            return ret;
        }
